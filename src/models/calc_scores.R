...
calc_Gini <- function(actual, pred) {
  ...
  results <- results %>% arrange(desc(pred, idx)) #dictate how ties sort
  ...
  accum_popn_percentage <- cumsum(rep(popn_increment, length(actual)))
...
calc_Gini_norm <- function(actual, pred) { calc_Gini(actual, pred) / calc_Gini(actual, actual) }

Gini_preds_tune <- function(tune, Yhat, Y) {
  
  out <- data.frame("gini" = calc_Gini(actual = Y, pred = Yhat))
  out <- bind_cols(out, tune)
  out
