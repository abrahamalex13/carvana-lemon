...
#predict
extract_preds_ranger <- function(mdl, newx) ranger:::predict.ranger(mdl, data = newx)$predictions[, 2]



validate_rangers <- function(mdl.l, X_train, Y_train, X_test, Y_test) {
  
  #predictions
  Yhat_train.l <- lapply(
    mdl.l, newx = X_train, function(x, newx) extract_preds_ranger(mdl = x$mdl, newx = newx)
  )  
  ...
  p <- 
    ggplot(confMat) +
      geom_line(aes(B, rate,
                    group = interaction(id_dataset, wgt_event),
                    linetype = id_dataset)) +
      geom_point(aes(B, rate,
                    group = interaction(id_dataset, wgt_event), 
                    shape = id_dataset)) +   
